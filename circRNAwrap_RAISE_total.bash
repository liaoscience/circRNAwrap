...
rm unmapped.title
$hisat2 -p 8 --mm --dta -x ./sailfish/index/total.repeat -U $sample.unmap.fq -S ./sailfish/$sample.unmap.sam 2> $sample.circmap.rate 
$samtools sort -@ 8 -o ./sailfish/$sample.unmap.bam ./sailfish/$sample.unmap.sam
$samtools rmdup -S ./sailfish/$sample.unmap.bam --reference ./sailfish/index/total.repeat ./sailfish/rmdup.bam
$bedtools bamtobed -split -bed12 -i ./sailfish/rmdup.bam > ./sailfish/rmdup.bed
#sed -i 's/__/\//g' ./sailfish/rmdup.bed
$bedtools bamtobed -split -bed12 -i ./sailfish/$sample.unmap.bam > ./sailfish/accepted_hits.bed
sed -i 's/__/\//g' ./sailfish/accepted_hits.bed
$bedtools intersect -split -wa -wb -abam ./sailfish/$sample.unmap.bam -b ./sailfish/index/ref2bp.bed -bed | awk '!a[$0]++' > circ_splice.reads
...
awk -F '[_\t]' 'NR==FNR{a[$1"\t"$4]=$0;next}NR>FNR{if($1"\t"$4 in a)print a[$1"\t"$4]"\t"$0}' circ_splice.reads ./sailfish/accepted_hits.bed | awk -F '[_\t]' '{if($4==$24 && $6!~$26) print }' > tmp.both_circ_splice.reads
...
################# 4 remove duplication reads and get the abundance
cd $dir/$sample
$bedtools intersect -split -wa -wb -abam ./sailfish/rmdup.bam -b ./sailfish/index/ref2bp.bed -bed | awk '!a[$0]++' > unique.circ_splice.reads     
...
#count
#both paired reads in circRNA sequence and they are not align to genome
awk -F '[_\t]' 'NR==FNR{a[$1"\t"$4]=$0;next}NR>FNR{if($1"\t"$4 in a)print a[$1"\t"$4]"\t"$0}' unique.circ_splice.reads ./sailfish/rmdup.bed | awk -F '[_\t]' '{if($4==$24 && $6!~$26) print }' > tmp.both_unique.circ_splice.reads
...
$mosdepth -t 8 --by $sample.tmp.2.circ.bed $sample.tmp.mos $sample.hisat2.rmdup.bam
...
################# 6 circRNA backsplice site cis splice reads abundance
cd $dir/$sample
